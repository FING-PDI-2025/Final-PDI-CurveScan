...
def spline_fit_and_sample(contour, num_points=50, input_resolution=100, smoothing=3):
    ...
    if len(unique_points) < 4:
        raise ValueError(
            "Not enough unique points for spline fitting. Adjust input_resolution or smoothing."
        )
    ...
    sampled_points = np.column_stack(out)
    return sampled_points


def get_sorted_quadrilateral_corners(contour):
    """
    ...
    length_01_23 = np.linalg.norm(pair_01_23[1] - pair_01_23[0]) + np.linalg.norm(
        pair_01_23[2] - pair_01_23[3]
    )
    length_12_30 = np.linalg.norm(pair_12_30[1] - pair_12_30[0]) + np.linalg.norm(
        pair_12_30[2] - pair_12_30[3]
    )
...
def is_contour_clockwise(contour):
    """
...
def enforce_clockwise_contour(contour):
    """
...
def get_nearest_index_in_contour(contour, point):
    """
...
def split_contour_edges(contour, corner_indices):
    """
...
def sample_contour_edges_with_splines(edges, points_per_side, smoothing):
    all_samples = []
    for edge in edges:
        ...
        samples = spline_fit_and_sample(
            edge, num_points=points_per_side, smoothing=smoothing
        )
        all_samples.append(samples)
    return np.vstack(all_samples)


def sample_contour_edges(edges, points_per_side, smoothing=3, method="spline"):
    all_samples = []
...
def generate_destination_rectangle(corners, points_per_side):
    width = np.mean(
        [
...
def plot_split_edges_colored(
    edges, corners, title="Split Contour Edges", background=None
):
    """
    ...
    """
    assert len(edges) == 4, "Expected 4 contour edge segments"
    ...
    for i, corner in enumerate(corners):
        plt.scatter(
            corner[0], corner[1], color="lime", s=20, label=f"{corner_labels[i]}"
        )
...
def prepare_tps_points_from_mask(
    mask, points_per_side=20, smoothing=3, sampling_method="spline"
):
    ...
    nearest_index_per_corner = [
        get_nearest_index_in_contour(contour, corner) for corner in corners
    ]
    nearest_index_per_corner = [
        *nearest_index_per_corner,
    ...
    src_points = sample_contour_edges(
        edges, points_per_side, smoothing=smoothing, method=sampling_method
    )
    dst_points, rect_dims = generate_destination_rectangle(corners, points_per_side)
    return src_points, dst_points


def apply_tps_transform(image, src_points, dst_points, output_shape):
    assert image is not None and image.dtype == np.uint8 and image.ndim == 3
    assert src_points.shape == dst_points.shape and len(src_points) >= 3
    src_points = src_points.reshape(-1, 1, 2).astype(np.float32)
    dst_points = dst_points.reshape(-1, 1, 2).astype(np.float32)
    ...
    # warped = tps.warpImage(image, flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
    warped = tps.warpImage(image)

    # assert output_shape[0] > 0 and output_shape[1] > 0
    # warped_resized = cv2.resize(warped, output_shape)

    return warped


def apply_tps_transform_skimage(image, src_points, dst_points, output_shape=None):
...
def process_image(
    ...
    # plot_tps_input(dst_points, original_image, src_points)
...
def plot_tps_input(dst_points, original_image, src_points):
    ...
    plt.imshow(original_image, alpha=0.5, zorder=-1)
    plt.scatter(
        src_points[:, 0], src_points[:, 1], color="blue", label="Source Points", s=2
    )
    plt.scatter(
        dst_points[:, 0], dst_points[:, 1], color="red", label="Destination Points", s=2
    )
...
if __name__ == "__main__":
    original_image_path = (
        ...
        / "datasets"
        / "creative"
        / "redbg_3_noflash_light.jpeg"
    )
    mask_image_path = (
        Path(__file__).parent.parent / "outputs" / "redbg_3_noflash_light_mask_1.jpeg"
    )

    # original_image_path = Path(__file__).parent.parent / "datasets" / "hojas-impresas"/ "bluebg_warm_noflash_light.jpg"
    # mask_image_path = Path(__file__).parent.parent / "outputs" / "bluebg_warm_noflash_light_mask_0.jpeg"

    original_image_path = (
        Path(__file__).parent.parent
        / "datasets"
        / "hojas-impresas"
        / "bluebg_double_noflash_light.jpg"
    )
    mask_image_path = (
        Path(__file__).parent.parent
        / "outputs"
        / "bluebg_double_noflash_light_mask_1.jpeg"
    )
